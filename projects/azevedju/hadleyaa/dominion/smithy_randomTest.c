...
* Section: 400
* Description: Random test suite for the smithy card refactored in
* dominion.c
********************************************************************/
#include "dominion.h"
...
//Description: Copies the randState into a preState struct. randState is modified with the smithy function. 
...
int randomizeGame(struct gameState *randState){ 
	...
	player = rand() % randState->numPlayers;
...
int main () {
	...
	printf("Beginning Random Testing for Smithy Effect.\n");
	printf("-------------------------------------------------------\n\n");
	...
	for(i = 0 ; i < 200 ; ++i){
		...
		passFail = testSmithy(&randState, player, i);
	}
	

	printf("ALL TESTS COMPLETE \n");
	if(passFail == 0){
		printf("SMITHY FAILED TESTS\n\n");
	}
	else{
