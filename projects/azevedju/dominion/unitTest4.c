/*************************************************************
Unit tests for adventurerEffect function
**************************************************************/
#include "dominion.h"
#include "dominion_helpers.h"
#include <string.h>
#include <stdio.h>
#include <assert.h>
#include "rngs.h"
#include <stdlib.h>

void assertTrue(int testResult, int correctResult){
	if(testResult == correctResult){
		printf("Passed\n\n");
	}
	else{
		printf("Failed\n\n");
	}
}

int main() {

	int currentPlayer = 0; 
	int otherPlayer = 1;
	int numPlayers = 2;
	int seed = 100;
	int k[10] = {adventurer, embargo, village, minion, mine, cutpurse,
			sea_hag, tribute, smithy, council_room};
	int cardsDrawn;
	int cardsDiscarded;
	struct gameState state;
	struct gameState testState; 

	//Initialize a game state condition
	initializeGame(numPlayers, k, seed, &state);

	printf("--------------- Testing adventurerEffect --------------- \n");

	//------------------- TEST 1: Player draws two cards ----------------
	memcpy(&testState, &state, sizeof(struct gameState));
	adventurerEffect(currentPlayer, &testState);

	printf("TEST 1: Player draws two additional cards \n");
	cardsDrawn = 2;
	cardsDiscarded = 1;
	printf("Hand count = %d, expected = %d\n", testState.handCount[currentPlayer], state.handCount[currentPlayer] + cardsDrawn - cardsDiscarded);
	assertTrue(testState.handCount[currentPlayer], state.handCount[currentPlayer] + cardsDrawn - cardsDiscarded);

	//------------------ TEST 2: Cards drawn are treasure cards --------
	memcpy(&testState, &state, sizeof(struct gameState));
	adventurerEffect(currentPlayer, &testState);

	printf("TEST 2: Drawn cards are treasure cards\n");
	int i; 
	int treasureCards = 0;
	int test_treasureCards = 0;

	//Count number of treasure cards in test state
	cardsDrawn = 0;
	for(i = 0 ; i < testState.handCount[currentPlayer] ; i++){
			cardsDrawn = testState.hand[currentPlayer][i];
			if(cardsDrawn == copper || cardsDrawn == silver || cardsDrawn == gold){
				test_treasureCards++;
			}
	}
	
	//Count number of treasure cards in control state
	cardsDrawn = 0;
	for(i = 0 ; i < state.handCount[currentPlayer] ; i++){
			cardsDrawn = state.hand[currentPlayer][i];
			if(cardsDrawn == copper || cardsDrawn == silver || cardsDrawn == gold){
				treasureCards++;
			}
	}

	printf("Treasure Cards = %d, expected = %d\n", test_treasureCards, treasureCards + 2);	//Add two to treasureCards for expected outcome.
	assertTrue(test_treasureCards, treasureCards + 2); 


	//-------------------- TEST 3: Player draws cards from own deck --------- 
	memcpy(&testState, &state, sizeof(struct gameState));
	adventurerEffect(currentPlayer, &testState);

	printf("TEST 3: Player draws from own deck\n");
	int testDeckTotal = testState.handCount[currentPlayer] + testState.discardCount[currentPlayer] + testState.deckCount[currentPlayer] ;
	int controlDeckTotal = state.handCount[currentPlayer] + state.discardCount[currentPlayer] + state.deckCount[currentPlayer] ;

	printf("Total deck count = %d, expected = %d\n", testDeckTotal, controlDeckTotal);
	assertTrue(testDeckTotal, controlDeckTotal);

	//-------------------- TEST 4: No cards drawn from opponents deck -------
	memcpy(&testState, &state, sizeof(struct gameState));
	adventurerEffect(currentPlayer, &testState);

	printf("TEST 4: No cards drawn from opponents deck\n");
	printf("Deck count = %d, expected = %d\n", testState.deckCount[otherPlayer], state.deckCount[otherPlayer]);
	assertTrue(testState.deckCount[otherPlayer], state.deckCount[otherPlayer]);


	//-------------------- TEST 5: Test that other player does not draw a card ------------
	memcpy(&testState, &state, sizeof(struct gameState));
	adventurerEffect(currentPlayer, &testState);

	printf("TEST 5: Check that other player does not draw a card\n");
	printf("Hand count = %d, expected = %d\n", testState.handCount[otherPlayer], state.handCount[otherPlayer]);
	assertTrue(testState.handCount[otherPlayer], state.handCount[otherPlayer]);

	//-------------------- TEST 6: Test for change in supply pile -----------------
	memcpy(&testState, &state, sizeof(struct gameState));
	adventurerEffect(currentPlayer, &testState);

	printf("TEST 6: Check for change in supply pile\n");
	int testSupply = 0; 
	int stateSupply = 0;
	for(i = 0 ; i < 25 ; i++){
		testSupply += testState.supplyCount[i] ;
		stateSupply += state.supplyCount[i];
	}
	printf("Supply count = %d, expected = %d\n", testSupply, stateSupply);
	assertTrue(testSupply, stateSupply);


//printf("<<<<<<<<<<<<<<< End of adventurerEffect() unit test >>>>>>>>>>>>>>>\n\n");

return 0;
}